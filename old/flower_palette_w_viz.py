...
### phase 1: handling monochromatic mode first, because this is a very specific mode which should be handled separately.
    if mode == "monochromatic":
        if base_color is None:
            return []
        return color_groups.get(base_color, [])
### phase 2: bif the mode is not monochormatic, the function goes to the palette logic
    palette = color_modes.get(mode, [])
### validate base color belongs to one of the groups in the palette
    base_group = None
    if base_color is not None:
        ...
        if base_group not in palette:
            return []
### phase 3: generate final colour recommendation
    expanded = []
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.H2("📈 Flower Seasonality Calendar"),

        dcc.Dropdown(
            id="seasonality-filter",
            options=[
                {"label": f, "value": f}
                for f in sorted(df_flowers["flower_type"].unique())
            ],
...
# 🌸 1. SEASONALITY CHART CALLBACK
@app.callback(
    Output("seasonality-chart", "figure"),
    Input("seasonality-filter", "value")
)
def update_seasonality_chart(selected_type):
    ...
    fig = px.scatter(
        ...
        title="Flower Availability by Season"
    )
...
def update_color_options(focal):
    ...
    if not focal:
        all_colors = [
            c for group in color_groups.values()
            for c in group
        ]
